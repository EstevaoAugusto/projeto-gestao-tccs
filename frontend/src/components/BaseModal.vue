...
<template>
  <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-zinc-900/40 px-4 pt-16 pb-8" @mousedown.self="emit('close')">
    <div
      class="w-full rounded-lg border border-zinc-200 bg-white text-zinc-900 shadow-sm [color-scheme:light]"
      :class="wide ? 'max-w-2xl' : 'max-w-lg'"
      role="dialog"
      aria-modal="true"
    >
      <header class="flex items-center justify-between gap-4 border-b border-zinc-200 px-6 py-4">
        <h2 class="min-w-0 text-base font-semibold text-zinc-900">{{ title }}</h2>
        <button
          type="button"
      ...
      <div class="px-6 py-4 text-zinc-700">
        <slot />
      </div>
